...
async def verified_user_and_token(db_session):
    """
    Fixture to create a verified user and generate an access token for them.

    This fixture:
    - Adds a user with AUTHENTICATED role to the database.
    - Sets email verification to True and the account to unlocked.
    - Generates a short-lived access token for authentication purposes.

    Returns:
        A tuple containing the user object and the access token.
    """
    # Prepare user data
    new_user_data = {
        "email": fake.email(),
    ...
    verified_user = User(**new_user_data)
    db_session.add(verified_user)
    await db_session.commit()

    # Create an access token for the new user
    access_token_payload = {
        "sub": str(verified_user.email),
        "role": verified_user.role.name,
    }
    access_token = create_access_token(
        data=access_token_payload, expires_delta=timedelta(minutes=30)
    )
